...
describe('ReadingStatsTracker', () => {
  ...
  it('contains a database-open failure instead of leaking an unhandled rejection', async () => {
    ...
    // FORK: the tracker never opens the statistics DB without a non-transient
    // book (the bookMd5 pin, guarded by fork-pins.test.ts), so hand it one for
    // the open — and its synthetic failure — to happen at all.
    mocks.getBookData.mockReturnValue({
      book: { hash: 'md5-open', title: 'Book', author: 'Author' },
    } as unknown as null);
    ...
    mocks.getBookData.mockReturnValue(null);
  });
});
